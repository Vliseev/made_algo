...
namespace TestRunnerPrivate {
template <typename K, typename... TArgs, template <typename...> class Map>
std::ostream &PrintUnorderedMap(std::ostream &os, const Map<K, TArgs...> &m) {
    os << "{";
    bool first = true;
    for (const auto &kv : m) {
        if (!first) {
...
std::ostream &operator<<(std::ostream &os, const std::unordered_map<K, V> &m) {
    return TestRunnerPrivate::PrintUnorderedMap(os, m);
}
